...
export const AreaDetailPage: React.FC = () => {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return areaData.assessments.filter(
      (user) =>
  ...
  const getUserChartData = (
    ...
    return sceneConfig.map((config) => {
      ...
      return {
        ...
        time: scoreItem ? scoreItem.time : undefined,
      };
    });
  };
        ...
        <div className="flex flex-col md:flex-row gap-4 items-end md:items-center w-full xl:w-auto">
          <div className="flex flex-col items-end gap-1">
            ...
            <ToggleSwitch
              ...
              onChange={(checked) =>
                setViewMode(checked ? "score" : "attempts")
              }
            />
          </div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {paginatedUsers.map((user) => {
              ...
              const averagePX = Math.round(
                userChartData.reduce((acc, c) => acc + c.value, 0) /
                  userChartData.length
              );
              const totalAttempts = userChartData.reduce(
                (acc, c) => acc + c.attempt,
                0
              );
              const averageAttempts =
                totalAttempts / (userChartData.length || 1);
                  ...
                  <div className="p-4 border-b border-slate-800 bg-slate-900/50 flex items-start justify-between">
                    ...
                    <div className="flex flex-col items-end">
                      {viewMode === "score" ? (
                        ...
                        <div
                          className={`text-sm font-bold mt-1 px-2 py-0.5 rounded bg-slate-800 border border-slate-700 ${
                            totalAttempts === 0
                              ? "text-slate-400"
                              : roundedAttempts === 1
                              ? "text-emerald-400"
